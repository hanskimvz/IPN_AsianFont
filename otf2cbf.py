...
def rasteriseChar(face, c):
    ...
    for y in range(height):
        ...
        #   bits[y * width + x] = (buffer[x / 8 + rowStart] & (0x80 >> (x % 8)))
            v =  buffer[x / 8 + rowStart] & (0x80 >> (x % 8))
            # bits[y * width + x] = (buffer[x / 8 + rowStart] & (0x80 >> (x % 8))) &1
            bits[y * width + x] = 1 if v else 0
...
# chars :  {'bbx': (1, 4, 8, 2), 'dwidth': (12, 12), 'bitmap': bitarray('1001')}
    ...
    charHuffLuts = map(lambda c: makeHuffLutCanonical( makeHuffLut(buildHuffTree(c))), counts)
...
def main():
    ...
    # Parse the map file
    # print (mapFilePath)
    print(CommentedFile(open(mapFilePath, "rb")))
    ...
    # for ch in chars:
    #     if ch.__dict__:
    #         print ('bbx', ch.bbx, ',dwidth', ch.dwidth, ',bitmap', ch.bitmap)
